...
async function meters(){
 ...
 $(" #app".trim()).innerHTML="<h2>บันทึกหน่วยน้ำ-ไฟ</h2><div class='panel'><h3>บันทึกเลขมิเตอร์ประจำเดือน</h3><p class='muted'>กรอกเลขมิเตอร์และกำหนดราคาต่อหน่วยเองได้ในแต่ละเดือน</p><form onsubmit='saveMeter(event)'><div class='grid meter-grid'><label>ห้อง<select name='room_id' required>"+r.map(x=>"<option value='"+x.id+"'>ห้อง "+x.room_no+"</option>").join("")+"</select></label><label>รอบเดือน<input name='reading_month' type='month' value='"+current+"' required></label></div><div class='meter-section'><h3>⚡ ค่าไฟ</h3><div class='grid'><label>เลขครั้งก่อน<input name='electricity_prev' type='number' step='0.01' min='0' value='0'></label><label>เลขปัจจุบัน<input name='electricity_current' type='number' step='0.01' min='0' value='0'></label><label>ราคา/หน่วย (บาท)<input name='electricity_rate' type='number' step='0.01' min='0' value='5' required></label></div></div><div class='meter-section'><h3>💧 ค่าน้ำ</h3><div class='grid'><label>เลขครั้งก่อน<input name='water_prev' type='number' step='0.01' min='0' value='0'></label><label>เลขปัจจุบัน<input name='water_current' type='number' step='0.01' min='0' value='0'></label><label>ราคา/หน่วย (บาท)<input name='water_rate' type='number' step='0.01' min='0' value='10' required></label></div></div><label>หมายเหตุ<input name='note'></label><button>บันทึกและคำนวณ</button></form></div><div class='panel'><h3>ประวัติหน่วยน้ำ-ไฟ</h3><table><tr><th>เดือน</th><th>ห้อง</th><th>ใช้ไฟ</th><th>ค่าไฟ</th><th>ใช้น้ำ</th><th>ค่าน้ำ</th><th>รวม</th><th>จัดการ</th></tr>"+m.map(x=>{const eu=Math.max(0,Number(x.electricity_current)-Number(x.electricity_prev)),wu=Math.max(0,Number(x.water_current)-Number(x.water_prev)),ec=eu*Number(x.electricity_rate||0),wc=wu*Number(x.water_rate||0);return "<tr><td>"+x.reading_month+"</td><td>"+x.room_no+"</td><td>"+eu.toLocaleString("th-TH")+" หน่วย</td><td>"+money(ec)+" ฿ <small>("+x.electricity_rate+" ฿/หน่วย)</small></td><td>"+wu.toLocaleString("th-TH")+" หน่วย</td><td>"+money(wc)+" ฿ <small>("+x.water_rate+" ฿/หน่วย)</small></td><td><b>"+money(ec+wc)+" ฿</b></td><td><button onclick='deleteMeter("+x.id+")'>ลบ</button></td></tr>"}).join("")+"</table></div>";
}
async function saveMeter(e){e.preventDefault();const f=new FormData(e.target);const d=Object.fromEntries(f.entries());d.room_id=Number(d.room_id);["electricity_prev","electricity_current","water_prev","water_current","electricity_rate","water_rate"].forEach(k=>d[k]=Number(d[k]||0));if(d.electricity_current<d.electricity_prev||d.water_current<d.water_prev){alert("เลขปัจจุบันต้องไม่น้อยกว่าเลขครั้งก่อน");return}const r=await send("/api/meter-readings","POST",d);if(!r.ok){alert(r.error||"บันทึกไม่สำเร็จ");return}await meters()}
...
async function bills(){
 ...
 $("#app").innerHTML="<h2>บิลและการชำระเงิน</h2><div class='panel'><h3>ส่งออกประวัติการจ่ายเงินเป็น Excel</h3><label>เลือกเดือน<input id='exportMonth' type='month' value='"+exportMonth+"'></label> <button onclick='exportPayments()'>ดาวน์โหลด Excel</button></div><div class='panel'><h3>สร้างบิล</h3><form onsubmit='addBill(event)'><div class='grid'><label>ผู้เช่า<select name='tenant_id' required>"+t.map(x=>"<option value='"+x.id+"'>ห้อง "+(x.room_no||"-")+" - "+x.name+"</option>").join("")+"</select></label><label>เดือน<input type='month' name='bill_month' value='"+new Date().toISOString().slice(0,7)+"'></label><label>ค่าเช่า<input name='rent' type='number' value='2800'></label><label>ค่าไฟ<input name='electricity' type='number' value='0'></label><label>ค่าน้ำ<input name='water' type='number' value='0'></label><label>อื่นๆ<input name='other' type='number' value='0'></label></div><button>สร้างบิล</button></form></div><div class='panel'><table><tr><th>เดือน</th><th>ห้อง</th><th>ผู้เช่า</th><th>ยอด</th><th>สถานะ</th><th>จัดการ</th></tr>"+b.map(x=>"<tr><td>"+x.bill_month+"</td><td>"+x.room_no+"</td><td>"+x.name+"</td><td>"+money(x.total)+"</td><td>"+billStatus(x.status)+"</td><td>"+(x.status==='ชำระแล้ว'?'เรียบร้อย':"<button onclick='payBill("+x.id+","+x.total+")'>รับชำระ</button> <button onclick='sendBill("+x.id+")'>ส่ง LINE</button> <button onclick='deleteBill("+x.id+")'>ลบบิล</button>")+"</td></tr>").join("")+"</table></div>"
}
async function exportPayments(){const m=$("#exportMonth").value||new Date().toISOString().slice(0,7);const r=await fetch("/api/export/payments?month="+encodeURIComponent(m),{headers:{Authorization:"Bearer "+token}});if(!r.ok){alert("ดาวน์โหลด Excel ไม่สำเร็จ");return}const blob=await r.blob();const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="payments-"+m+".xlsx";a.click();URL.revokeObjectURL(a.href)}
...
async function deleteBill(id){if(!confirm("ต้องการลบบิลนี้ใช่ไหม? บิลที่ลบจะหายเฉพาะรายการนี้ค่ะ"))return;const r=await send("/api/bills/"+id,"DELETE",{});if(!r.ok){alert(r.error||"ลบบิลไม่สำเร็จ");return}await bills()}
